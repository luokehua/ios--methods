...
#define STARTPOINT CGPointMake(160, 200)
...
- (id)initWithFrame:(CGRect)frame menus:(NSArray *)aMenusArray
{
    ...
    if (self) {
        ...
        for (int i = 0; i < count; i ++)
        {
            ...
            item.endPoint = CGPointMake(STARTPOINT.x - ENDRADIUS * sinf(i * M_PI * 2 / (count - 1)), STARTPOINT.y - ENDRADIUS * cosf(i * M_PI * 2 / (count - 1)));
            item.nearPoint = CGPointMake(STARTPOINT.x - NEARRADIUS * sinf(i * M_PI * 2 / (count - 1)), STARTPOINT.y - NEARRADIUS * cosf(i * M_PI * 2 / (count - 1)));
            item.farPoint = CGPointMake(STARTPOINT.x - FARRADIUS * sinf(i * M_PI * 2 / (count - 1)), STARTPOINT.y - FARRADIUS * cosf(i * M_PI * 2 / (count - 1)));
...
- (void)setExpanding:(BOOL)expanding
{
    ...
    // rotate add button
    float angle = self.isExpanding ? M_LN10 : 0.0f;
    [UIView animateWithDuration:0.2f animations:^{
        _addButton.transform = CGAffineTransformMakeRotation(angle);
    }];
